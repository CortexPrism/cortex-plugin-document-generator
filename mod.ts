...
const doc_generate: Tool = {
  ...
  execute: async (args: Record<string, unknown>, _ctx: PluginContext): Promise<ToolCallResult> => {
    ...
    try {
      ...
      const result = `Generated ${format.toUpperCase()} document: "${title}" by ${author}${
        include_toc ? ' (with TOC)' : ''
      }`;
      return {
        toolName: 'doc_generate',
        success: true,
...
const doc_template_list: Tool = {
    ...
    params: [
      {
        ...
        options: ['report', 'proposal', 'letter', 'meeting_notes', 'resume'],
        required: false,
      },
    ],
    capabilities: [],
  ...
  execute: async (args: Record<string, unknown>, _ctx: PluginContext): Promise<ToolCallResult> => {
    ...
    try {
      ...
      if (category === 'all') {
        const all = Object.entries(templates).map(([cat, tmpls]) => `${cat}: ${tmpls.join(', ')}`)
          .join('\n');
        return {
          toolName: 'doc_template_list',
          success: true,
          output: all,
          durationMs: Date.now() - start,
        };
    ...
    try {
      ...
      try {
        ...
        return {
          ...
          success: false,
          output: '',
          error: 'files must be valid JSON array',
          durationMs: Date.now() - start,
        };
      }

      const result =
        `Merged ${fileList.length} file(s) into ${output_format.toUpperCase()}: ${output_path}`;
      return {
        toolName: 'doc_merge',
        success: true,
        output: result,
